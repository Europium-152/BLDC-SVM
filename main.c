...
int g_ang=0,           //Converted angle, stores the angle in units of its resolution, 
    ...
    g_m=2,
    print3=0, print2=0,
        g_PDC1=0, g_PDC2=0, g_PDC3=0
;             //VDC divisor, g_m=x means max tension is equal to V_unreg/x
...
int main(void) {
    ...
    while(1){
        ...
        if (pr2_old<460 && pr2_old>400 && print2==1){
            printf("PDC1  %d   PDC2  %d   PDC3  %d        pr2  %d \n", PDC1, PDC2, PDC3, pr2_old);
            print2=0;
        }
        if(PR2<460)
            PR2=PR2;
...
void TIMER2_config(){
    ...
    T2CONbits.TCKPS = 0x11; //sets the Timer_2 pre-scaler to 1
